...
{
	float3 pos = inPos;
	pos.xz = (frac(inPos.xz / 6) - 0.6) * 6;
	pos.y += -2;

	float2 res = float2(1e10, 0.0);

	//UFO
	res = opU(float2(sdCylinder(pos - float3(-2.0, 0.5, 1.0), float2(0.6, 0.05)), 100), 
		float2(sdCylinder(pos - float3(-2.0, 0.47, 1.0), float2(0.5, 0.04)), 100));
	res = opU(res, float2(opS(sdCylinder(pos - float3(-2.0, 0.44, 1.0), float2(0.4, 0.04)), sdCylinder(pos - float3(-2.0, 0.1, 1.0), float2(0.15, 0.5))), 100));
	res = opU(res, float2(opS(sdCylinder(pos - float3(-2.0, 0.41, 1.0), float2(0.3, 0.04)), sdCylinder(pos - float3(-2.0, 0.1, 1.0), float2(0.15, 0.5))), 100));
	res = opU(res, float2(opS(sdCylinder(pos - float3(-2.0, 0.38, 1.0), float2(0.2, 0.04)), sdCylinder(pos - float3(-2.0, 0.1, 1.0), float2(0.15, 0.5))), 100));
	res = opU(res, float2(opS(sdCylinder(pos - float3(-2.0, 0.53, 1.0), float2(0.5, 0.04)), sdCylinder(pos - float3(-2.0, 0.1, 1.0), float2(0.15, 0.5))), 100));
	res = opU(res, float2(opS(sdSphere(pos - float3(-2.0, 0.5, 1.0), 0.25), sdCylinder(pos - float3(-2.0, 0.1, 1.0), float2(0.15, 0.5))), 60));

	//Martian head
	float2 res2 = float2(1e10, 0.0);
	res2 = opU(res2, float2(softMin2(sdCapsule(pos - float3(0.0, 0.4, 1.0), float3(0.2, 0.0, 0.0), float3(-0.2, 0.0, 0.0), 0.1),
		sdCapsule(pos - float3(0.0, 0.2, 1.0), float3(0.6, 0.0, 0.0), float3(-0.6, 0.0, 0.0), 0.1), 1), 100));
	res2 = opU(res2, float2(softMin2(res2, sdCapsule(pos - float3(0.0, 0.0, 1.0), float3(1.0, 0.0, 0.0), float3(-1.0, 0.0, 0.0), 0.1), 1), 100));
	res2 = opU(res2, float2(softMin2(res2, sdCapsule(pos - float3(0.0, -0.2, 1.0), float3(0.6, 0.0, 0.0), float3(-0.6, 0.0, 0.0), 0.1), 1), 100));

	//Eyeballs
	res2 = opU(res2, float2(sdSphere(pos - float3(0.6, 0.05, 0.8), 0.2), 10));
	res2 = opU(res2, float2(sdSphere(pos - float3(0.0, 0.2, 0.77), 0.2), 10));
	res2 = opU(res2, float2(sdSphere(pos - float3(-0.6, 0.05, 0.8), 0.2), 10));

	//Pupils
	res2 = opU(res2, float2(sdSphere(pos - float3(0.625, 0.05, 0.65), 0.1), 80));
	res2 = opU(res2, float2(sdSphere(pos - float3(0.0, 0.2, 0.6), 0.1), 80));
	res2 = opU(res2, float2(sdSphere(pos - float3(-0.625, 0.05, 0.65), 0.1), 80));

	//Antenna?
	res2 = opU(res2, float2(sdTorus(opTwist(pos - float3(0.0, 0.8, 1.0)), float2(0.2, 0.05)), 100));
